...
class Mage(name: String, maxHp: Int = 80) : Hero(name, maxHp) {
    ...
    fun lightningBolt(target: Enemy) {
        ...
        target.hp -= dmgAmnt
        println("    $white>>>$reset $bold$blue2$name$reset deals $yellow2$dmgAmnt dmg$reset to $bold$red2${target.name}$reset with $bold${blue1}Lightning Bolt$reset $white<<<$reset")
        Thread.sleep(200)
    ...
    fun magicMissiles(targets: MutableList<Enemy>) {
        val dmgAmnt1 = ((20..35).random() * skillMod).roundToInt()
        val target1 = targets.filter { it.hp > 0 }.random()
        target1.hp -= dmgAmnt1
        val dmgAmnt2 = ((20..35).random() * skillMod).roundToInt()
        val target2 = targets.filter { it.hp > 0 }.random()
        target2.hp -= dmgAmnt2
        println("    $white>>>$reset $bold$blue2$name$reset deals $yellow2$dmgAmnt1 dmg$reset to $red2$bold${target1.name}$reset and $yellow2$dmgAmnt2 dmg$reset to $red2$bold${target2.name}$reset with $blue1${bold}Magic Missiles$reset $white<<<$reset")
        Thread.sleep(200)
        println(
            "                          $white>>>$reset $bold$red2${targets.map { it.name }}$reset now ${
                if (targets.size == 1) {
    ...
    fun searingTouch(target: Enemy) {
        if (!target.burning) {
            val dmgAmnt = (30 * skillMod).roundToInt()
            target.hp -= dmgAmnt
            target.burning = true
            println("    $white>>>$reset $bold$blue2$name$reset deals $yellow2$dmgAmnt dmg$reset to $red2$bold${target.name}$reset with $bold${blue1}Searing Touch$reset and sets them on fire $white<<<$reset")
            Thread.sleep(200)
            println("                          $white>>>$reset $bold$red2${target.name}$reset now has $green2${target.hp} hp$reset $white<<<$reset")
            deathCheck(target)
        }
