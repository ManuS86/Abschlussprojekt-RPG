...
class Mage(name: String, maxHp: Int = 80) : Hero(name, maxHp) {
    ...
    fun fireball(targets: MutableList<Enemy>) {
        ...
        println("    $white>>>$reset $bold$blue2$name$reset deals $red2${(30 * skillMod).roundToInt()} dmg$reset to $bold${yellow2}each enemy$reset with $bold${blue1}Fireball$reset $white<<<$reset")
        println(
