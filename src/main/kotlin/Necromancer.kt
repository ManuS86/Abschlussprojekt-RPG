...
class Necromancer(name: String, maxHp: Int = 250) : Enemy(name, maxHp) {
    fun deathWave(heroes: List<Hero>) {
        heroes.forEach { it.hp -= (30 * dmgModifier / it.durability).roundToInt() }
        println("$name deals ${heroes.forEach { (30 * dmgModifier / it.durability).roundToInt() }} dmg to each hero with Death Wave.")
    }

    fun blight(target: Hero) {
        val dmgAmount = (50 * dmgModifier / target.durability).roundToInt()
        target.hp -= dmgAmount
        println("$name deals $dmgAmount dmg to ${target.name} with Blight.")
    }

    fun vampiricTouch(target: Hero) {
        val dmgAmount = (30 * dmgModifier / target.durability).roundToInt()
        target.hp -= dmgAmount
        val preHealHp = hp
        heal((30 * dmgModifier / target.durability).roundToInt())
